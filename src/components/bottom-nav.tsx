...
export function BottomNav() {
  ...
  return (
    <nav className="fixed right-0 bottom-0 left-0 z-40 px-3 pb-[calc(env(safe-area-inset-bottom)+8px)]">
      <div className="mx-auto w-full max-w-xl rounded-t-3xl border border-[var(--nav-border)] bg-white/96 px-4 pt-2 shadow-[0_-8px_24px_rgba(15,23,42,0.08)] backdrop-blur-sm">
        <div className="grid grid-cols-3 items-end">
          <Link
            href="/"
            className={clsx(
              "flex h-14 flex-col items-center justify-center gap-1 rounded-2xl transition",
              albumActive
          ...
          >
            <AlbumIcon />
            <span className="text-[13px] font-semibold">图集</span>
          </Link>
          ...
          <Link
            ...
            aria-label="上传照片"
            className={clsx(
              "mx-auto -mt-7 flex h-[68px] w-[68px] items-center justify-center rounded-full border-4 border-white bg-[var(--accent)] text-[var(--accent-foreground)] shadow-[0_10px_20px_rgba(15,23,42,0.18)]",
          ...
          >
            <UserIcon />
            <span className="text-[13px] font-semibold">我的</span>
          </Link>
        </div>
      </div>
    </nav>
